    ...
    return (
        <AppLayout>
            ...
            <ProductsModal 
                ...
                className={`transition-all ${showModal ? 'mt-0 opacity-100 pointer-events-auto' : 'mt-5 opacity-0 pointer-events-none'}`} />
            ...
            <div className="mx-auto w-full max-w-7xl p-5">
                ...
                {selectedProducts.map((product) => (
                    <ProductsCalculatorTable key={product.id} product={product} styles={styles} />
                ))}
                ...
                <button 
                    className="m-5 rounded-lg bg-orange-600 py-2 px-3 text-white hover:bg-orange-700"
                    onClick={() => setShowModal(true)}>Select products</button>
            </div>
        </AppLayout>
    );
}
